...
int main() 
{
	...
	// Read video
	VideoCapture cap("../Video/RoadMarkings.mp4");
	if(!cap.isOpened())
	{
	...
	while(1)
	{
		...
		//string filename = "../OutputWarp/imgWarp" + to_string(imgCount) + ".jpg";
		//imwrite(filename, img_warp);
		//imshow("Warped Img", img_warp);

		// Draw ROI on img

		line(img_src, Point(541, 360), Point(694, 360), Scalar(0, 0, 255), 2, 8);
		line(img_src, Point(694, 360), Point(1190, 634), Scalar(0, 0, 255), 2, 8);
		line(img_src, Point(1190, 634), Point(1, 634), Scalar(0, 0, 255), 2, 8);
		line(img_src, Point(1, 634), Point(541, 360), Scalar(0, 0, 255), 2, 8);
...
Mat calculateHomography(void)
{
	...
	pts_src.push_back(Point2f(541, 360));
	pts_src.push_back(Point2f(694, 360));
	pts_src.push_back(Point2f(1190, 634));
	pts_src.push_back(Point2f(1, 634));

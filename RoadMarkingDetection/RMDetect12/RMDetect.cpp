...
int main() 
{
	...
	while(1)
	{
		...
		//string filename = "../Output2/img" + to_string(imgCount) + ".jpg";
...
int findMatch(Mat img_src, vector<Mat> templates, vector<Point> &matchPoints)
{
	...
	for(unsigned int i=0; i<templates.size(); i++)
	{
		...
		Mat img_result = templateMatch(img_src_edge, img_temp_edge);
		//Mat img_result;
...
// Method to calculate normalized cross correlation between template and image
Mat templateMatch(Mat img, Mat temp)
{

	Mat result(img.rows, img.cols, CV_64F, Scalar(0, 0, 0));
	...
	for(int y=0; y<=img.rows-temp.rows; y++)
	{
		for(int x=0; x<=img.cols-temp.cols; x++)
		{
			...
			//cout << "Current row = " << x+temp.rows << endl;
			//cout << "Current col = " << y+temp.cols << endl;

			// Extract part of image

			if( x+temp.cols > img.cols)
			{
				Rect R(x, y, img.cols-x, temp.rows);
				Rect RTemp(0, 0, img.cols-x, temp.rows);
				cout << "Inside 1: img.cols-x = " << img.cols-x << endl;
				newTemp = temp(RTemp);
				//cout << "After newTemp" << endl;
				roi = img(R);
				//cout << "After roi" << endl;
			}
			else if( y+temp.rows > img.rows)
			{
				Rect R(x, y, temp.cols, img.rows-y);
				Rect RTemp(0, 0, temp.cols, img.rows-y);
				cout << "Inside 2: img.rows-y = " << img.rows-y << endl;
				newTemp = temp(RTemp);
				roi = img(R);
			}
			else if( x+temp.cols > img.cols && y+temp.rows > img.rows )
			{
				Rect R(x, y, img.cols-x, img.rows-y);
				Rect RTemp(0, 0, img.cols-x, img.rows-y);
				cout << "Inside 3: img.cols-x = " << img.cols-x << endl;
				cout << "Inside 3: img.rows-y = " << img.rows-y << endl;
				newTemp = temp(RTemp);
				roi = img(R);
			}
			else
			{
				Rect R(x, y, temp.cols, temp.rows);
				newTemp = temp;
				roi = img(R);
			}
			...
			*/
			...
			// Element-wise matrix multiplication using mat.mul
			Mat elemMult = temp.mul(roi);

			// Square of temp and section of image
			Mat sqTemp = temp.mul(temp);
			Mat sqImg = roi.mul(roi);

			// Calculate normalized cross correlation
			//double result = sum(elemMult)[0] / sqrt(sum(sqTemp)[0] * sum(sqImg)[0]);
			//cout << "result = " << result << endl;
			result.at<double>(x, y) = (double)sum(elemMult)[0] / sqrt(sum(sqTemp)[0] * sum(sqImg)[0]);
			//cout << "tmp.at<float>(x, y) = " << tmp.at<float>(x, y) << endl;
		}
	}
